...
use App\Services\TryOttoService;
...
class WebController extends Controller
{
    ...
    public function __construct(
        ...
        private TryOttoService $tryOtto,
    ...
    public function getAllBrandsView(Request $request): View|RedirectResponse
    {
        ...
        if ($brandStatus == 1) {
            $brandList = Brand::active()->with([
                'brandProducts' => function ($query) {
    ...
    public function getCashOnDeliveryCheckoutComplete(Request $request): View|RedirectResponse|JsonResponse
    {
        ...
        if ($physicalProductExist) {
            ...
            $data = [
                ...
                "amount_due" => 0,
                "currency" => "SAR",
                "orderDate" => $currentDateTime->format('d/m/Y H:i'),
                "senderName" => "Al Bazar",
                "customer" => [
                    "name" => $userAddress?->contact_person_name,
                    "email" => $userAddress?->email,
                    "mobile" => $userAddress?->phone,
                    "address" => $userAddress?->address,
                    "city" => $userAddress?->city,
                    "country" => $userAddress?->country,
                    "postcode" => $userAddress?->zip,
                ...
                "items" => [
                    [
                        ...
                        "sku" => $product?->slug,
                        "image" => asset(json_decode($product->images)[0]->image_name),
                    ]
                ]
            ];
    ...
    public function getOrderPlaceView(Request $request): View
    {
        ...
        return view('themes.default.web-views.checkout.complete', [
            'order_ids' => $orderIds,
            'isNewCustomerInSession' => $isNewCustomerInSession,
        ]);
    }
